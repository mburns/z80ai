...
| `test_encoder_conformance.py` | One corpus of queries against **every** backend and the reference, flat and banded — an encoder that disagrees with training produces plausible buckets rather than an error, so this is what stands behind every change to one |
