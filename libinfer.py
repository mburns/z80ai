...
def rank(model: Model, query: str, accum_bits: int = 16,
    ...
    **Rank 2 holds the answer rank 1 missed 43.3% of the time.**  Over the
    silo's 3,240 held-out questions the top choice is right 66.8% of the time
    and the top two contain the answer 81.1% of the time, so a caller whose
    ...
    **The margin knows when it is wrong.**  The gap between the top two logits
    has a median of 86 where the answer is right and 26 where it is not, which
